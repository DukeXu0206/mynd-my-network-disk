window.addEventListener('DOMContentLoaded', function () {
    ...
    const pageMap = new Proxy({
        '#storage': {tab: storageTab, title: 'My Network Disk'},
        '#history': {tab: historyTab, title: 'Transfer History'},
        '#bin': {tab: binTab, title: 'Recycle Bin'},
        '#profile': {tab: profileTab, title: 'Personal Information'},
    }, {
        get(target, prop) {
    ...
    function initStorageTable() {
        ...
        breadcrumb.append('My Network Disk')
        ...
        function fileUpload() {
            ...
            if (used > terms.storage) {
                toast.setIcon(_fontawsome.warning).setText('Insufficient remaining space').show()
            } else if (size > _config.MAX_UPLOAD_FILE_SIZE) {
                toast.setIcon(_fontawsome.warning).setText(`A single upload cannot exceed${custom.fileSizeFormat(_config.MAX_UPLOAD_FILE_SIZE)}`).show()
            } else {
        ...
        function folderUpload() {
            ...
            if (used > terms.storage) {
                ...
                toast.setIcon(_fontawsome.warning).setText(`The number of uploaded items exceeds${_config.DATA_UPLOAD_MAX_NUMBER_FIELDS}limit`).show()
            } else {
                let formData = new FormData()
        ...
        function uploadPreHandler(url, name, data, used) {
            ...
            if (exist) toast.setIcon(_fontawsome.warning).setText('A file with the same name exists in the target folder').show()
        ...
        function uploadHandler(url, data, used) {
            ...
            xhr.upload.addEventListener('progress', (e) => {
                ...
                if (rate === '100%') toast.setIcon(_fontawsome.info).setText('Synchronizing file structure, please wait').show()
            })
            ...
            function cancel() {
                modal.setTitle('Are you sure you want to cancel the upload?')
    ...
    function initHistoryTable() {
        $historyTable.bootstrapTable({
            ...
            formatNoMatches: () => 'No transfer history',
            columns: [{
                checkbox: true,
            }, {
        ...
        function shareRemove(pks) {
            if (pks.length === 0) {
                ...
                modal.setTitle('Are you sure you want to delete the selected records?')
    ...
    function initBinTable() {
        $binTable.bootstrapTable({
            ...
            formatNoMatches: () => 'No recovery documents',
            columns: [{
                checkbox: true,
            }, {
        ...
        function fileRemove(pks, size) {
            if (pks.length === 0) {
                ...
                modal.setTitle('Are you sure you want to permanently delete the selected files?')
                    .setBtnType('btn btn-danger')
                    .once('show.mdb.modal', () => modal.getBtn().addEventListener('click', remove))
                    .once('hide.mdb.modal', () => modal.getBtn().removeEventListener('click', remove))
                    .show()
