window.addEventListener('DOMContentLoaded', function () {
    ...
    $.get(_urls.fileDetail(uuid), (file) => {
        ...
        const share = () => {
            $.get(_urls.fileShare(uuid), () => {
                toast.setIcon(_fontawsome.success).setText('Sharing record generated').show()
            })
        }
        const download = () => {
            location.href = _urls.fileBinary(uuid)
            toast.setIcon(_fontawsome.info).setText('Downloading, please wait').show()
        }
        ...
        infoEl.querySelector('p:nth-child(2)').textContent = `file name：${file.file_name}`
        infoEl.querySelector('p:nth-child(3)').textContent = `file type：${file.type}`
        infoEl.querySelector('p:nth-child(4)').textContent = `file size：${custom.fileSizeFormat(file.file_size)}`
        infoEl.querySelector('p:nth-child(5)').textContent = `upload time：${file.create_time}`
        ...
        if (isOverSize || isNotSupport) {
            ...
            if (isOverSize) {
                p.textContent = 'The file size exceeds the limit and cannot be previewed online'
            } else if (isNotSupport) {
                p.textContent = 'This type of file preview is not currently supported'
            }
        ...
        for (const [type, suffixes] of Object.entries(_config.media)) {
            if (suffixes.includes(file.type)) {
                $.ajax(_urls.fileBinary(uuid), {
                    ...
                    beforeSend: () => {
                        toast.setIcon(_fontawsome.info).setText('Loading resources').show()
                    },
                    success: (blob) => {
    ...
    function videoPlayer(file) {
        ...
        player.on('ready', () => toast.setIcon(_fontawsome.info).setText('Loading completed').show())
        player.on('error', () => toast.setIcon(_fontawsome.warning).setText('Failed to load').show())
    }
    ...
    function audioPlayer(file) {
        ...
        let howl = new Howl({
            ...
            onloaderror: () => toast.setIcon(_fontawsome.warning).setText('Failed to load').show(),
            onplayerror: () => toast.setIcon(_fontawsome.warning).setText('Playback failed').show(),
            onload: () => {
                ...
                toast.setIcon(_fontawsome.info).setText('Loading completed').show()
            }
        })
    ...
    function wordViewer(blob) {
        ...
        docx.renderAsync(blob, viewer).then(() => {
            ...
            toast.setIcon(_fontawsome.warning).setText('Failed to load').show()
        })
    }
